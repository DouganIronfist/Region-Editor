/****************************************************************************************************
 * 
 *   Filename    : RegionEditor.cs
 *   
 *   Description : Form that provides the main user interface
 *
...
{
    internal partial class RegionEditor : Form
    {
        #region Variables
        internal Bitmap[] RenderedMaps = new Bitmap[6] { null, null, null, null, null, null };
        ...
        #region Constructor
        internal RegionEditor()
        {
            InitializeComponent();
        }
        #endregion

        #region RegionEditor_FormClosing
        ...
        private void regionsxmlLocationToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Parameters.RegionsFile != "")
                dialog.FileName = Parameters.RegionsFile;
            else
                dialog.FileName = "regions.xml";
        ...
        private void scaleSlider_SliderMoveComplete(object sender, EventArgs e)
        {
            ...
            xSlider.SmallChange = (int)(mapDisplay.TileWidth * 0.9);
            ySlider.SmallChange = (int)(mapDisplay.TileWidth * 0.9);
        }
        ...
        private void facetView_DoubleClick(object sender, EventArgs e)
        {
            if (facetView.SelectedNode != null)
            {
                ...
                if (tag.Reference == "golocation")
                {
                    ...
                    GotoLocation(r.GoLocation.X, r.GoLocation.Y, 0, 0);
                }
                else if (tag.Reference == "entrance")
                {
                    mapDisplay.HighlightedArea = new Rectangle(r.Entrance.X, r.Entrance.Y, 1, 1);
                    GotoLocation(r.Entrance.X, r.Entrance.Y, 0, 0);
                }
                else if (tag.Reference == "regionarea")
                {
                    mapDisplay.HighlightedArea = r.Area[tag.Index].Area;
                    GotoLocation(r.Area[tag.Index].Area.X, r.Area[tag.Index].Area.Y, r.Area[tag.Index].Area.Width, r.Area[tag.Index].Area.Height);
                }
            }
        }
        ...
        {
            Parameters.MapIndex = facetList.SelectedIndex;
        ...
        internal void ForceRender(MapViewer viewer)
        {
            Select();
            viewer.Close();
        ...
        internal void GotoLocation(int x, int y, int w, int h)
        {
            GotoLocation(x, y, w, h, true);
        }

        internal void GotoLocation(int x, int y, int w, int h, bool center)
        {
            int displayX = (x + (w / 2) - (center ? (mapDisplay.TileWidth / 2) : 0));
            int displayY = (y + (h / 2) - (center ? (mapDisplay.TileWidth / 2) : 0));
        ...
        private void RegionEditor_Shown(object sender, EventArgs e)
        {
            ...
            compatible.Checked = Parameters.Compatibility;
            ...
            mapDisplay.MapScale = Scaling.Scale2;
            xSlider.SmallChange = (int)(mapDisplay.TileWidth * 0.9);
            ySlider.SmallChange = (int)(mapDisplay.TileWidth * 0.9);
            mapDisplay.Initialized = true;
